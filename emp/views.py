...
def feedback(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Manually handle form data since we're not using a ModelForm
            email = form.cleaned_data['email']
            name = form.cleaned_data['name']
            feedback = form.cleaned_data['feedback']
            
            # Save to database or process feedback here as needed

            return render(request, 'emp/feedback.html', {'form': form, 'success': True})
        else:
